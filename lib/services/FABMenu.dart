...
class FabMenu extends StatefulWidget {
  ...
  State<StatefulWidget> createState() {
    ...
    fabState =  _FabMenuState(centerBtn: _centerBtn, fabItems: _fabList,location: _location);
    return fabState;
  }
  ...
  FabMenu addCenterFab(IconData icon,
    ...
    _centerBtn = new CenterFab(icon: icon,tooltip: tooltip,mini: mini,closeColor: closeColor,menuCallback: ()=>fabState._changeMenu());
    return this;
  }
  ...
  FabMenu addActionFab(IconData icon,
    ...
    if (_fabList.length > 2) {
      throw FormatException('Expected up to two floatingActionButton');
    }
      ...
      builder: (BuildContext context, Widget child) {
        return CustomMultiChildLayout(
          delegate: _FabMenuDelegate(
              ...
              menuLocation:
              location == null ? MenuLocation.Centerdocked : location,
...
class _FabMenuDelegate extends MultiChildLayoutDelegate {
  ...
  Offset _calcMenuLocation(Size size, Size widget) {
    switch (this.menuLocation) {
      case MenuLocation.Centerdocked:
      default:
